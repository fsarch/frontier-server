...
export class UpstreamGroupController {
  constructor(private readonly upstreamGroupService: UpstreamGroupService) {}
  ...
  public async List(@Param('domainGroupId') domainGroupId: string) {
    const pathRules =
      await this.upstreamGroupService.ListByDomainGroupId(domainGroupId);
  ...
  public async Patch(
    ...
    const upstreamGroup = await this.upstreamGroupService.Update(
      id,
      upstreamGroupUpdateDto,
    );
    if (!upstreamGroup) {
