...
export class AddHookTableAndPathRuleHookIds1782845914875
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    const preHookIdColumn = pathRuleTable?.columns.find(
      (column) => column.name === 'pre_hook_id',
    );
    const postHookIdColumn = pathRuleTable?.columns.find(
      (column) => column.name === 'post_hook_id',
    );
